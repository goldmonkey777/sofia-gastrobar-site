# 🎵 Playlists do Spotify - Sofia Gastrobar Ibiza

## Playlists Configuradas

### ✅ Playlist Sunset Sessions (17h-21h)

**ID:** `5az1XeIPO0ijDQiz1nykRW`

**URLs:**
- **Embed:** `https://open.spotify.com/embed/playlist/5az1XeIPO0ijDQiz1nykRW?utm_source=generator`
- **Open:** `https://open.spotify.com/playlist/5az1XeIPO0ijDQiz1nykRW`

**Status:** ✅ Configurada

---

### ⏳ Playlist Night Vibes (21h-01h)

**ID:** `_________________` (aguardando)

**URLs:**
- **Embed:** `https://open.spotify.com/embed/playlist/SEU_ID_NIGHT?utm_source=generator`
- **Open:** `https://open.spotify.com/playlist/SEU_ID_NIGHT`

**Status:** ⏳ Pendente

---

### ⏳ Playlist Breakfast Flow (8h-12h)

**ID:** `_________________` (aguardando)

**URLs:**
- **Embed:** `https://open.spotify.com/embed/playlist/SEU_ID_BREAKFAST?utm_source=generator`
- **Open:** `https://open.spotify.com/playlist/SEU_ID_BREAKFAST`

**Status:** ⏳ Pendente

---

## 📝 Como Adicionar as Outras Playlists

1. Crie as playlists no Spotify
2. Torne-as **públicas** e **colaborativas**
3. Copie o link da playlist
4. Extraia o ID (parte depois de `/playlist/`)
5. Adicione no `.env.local`:

```env
NEXT_PUBLIC_SPOTIFY_NIGHT_EMBED_URL="https://open.spotify.com/embed/playlist/SEU_ID_NIGHT?utm_source=generator"
NEXT_PUBLIC_SPOTIFY_NIGHT_OPEN_URL="https://open.spotify.com/playlist/SEU_ID_NIGHT"

NEXT_PUBLIC_SPOTIFY_BREAKFAST_EMBED_URL="https://open.spotify.com/embed/playlist/SEU_ID_BREAKFAST?utm_source=generator"
NEXT_PUBLIC_SPOTIFY_BREAKFAST_OPEN_URL="https://open.spotify.com/playlist/SEU_ID_BREAKFAST"
```

---

## ✅ Checklist

- [x] Playlist Sunset Sessions criada e configurada
- [ ] Playlist Night Vibes criada e configurada
- [ ] Playlist Breakfast Flow criada e configurada
- [ ] Todas as playlists são públicas
- [ ] Todas as playlists são colaborativas
- [ ] IDs adicionados no `.env.local`
- [ ] Testado localmente
- [ ] Variáveis adicionadas no Vercel

---

**Última atualização:** Dezembro 2025

